...
export const onTransaction: OnTransactionHandler = async ({
  ...
  const url =
    "https://research.hapilabs.one/v1/snap/" +
    contract_address +
    "/" +
    chain_id +
    "/" +
  ...
  try {
    ...
    return {
      insights: {
        "Connection Error":
          "Unable to connect to snap backend. Please try again later.",
      },
    };
  }
  ...
  try {
    ...
    return {
      insights: {
        "Payload Error":
          "Unable to process request. Please contact the team and try again later.",
      },
    };
  }
  ...
  if (data.status == "ok") {
    ...
    return {
      insights: {
        "Unknown Error":
          "An unknown error occured. Please contact the team and try again later.",
      },
    };
  }
};
